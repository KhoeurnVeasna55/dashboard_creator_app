...
class CategoryController extends GetxController {
  ...
  Future<void> createCategory() async {
    ...
    try {
      ...
      clearForm();
      _categoryScreenController.goList();
      await load();
    } catch (e) {
      CustomToastNoti.show(
        title: 'Error',
  ...
  Future<void> loadCategoryForEditById(String id) async {
    try {
      loading.value = true;
      error.value = null;

      final category = await api.fetchCategoryById(id);
      loadCategoryForEdit(category);
      editingCategoryId.value = category.id;
      isEditing.value = true;
    } catch (e) {
      error.value = 'Failed to load category: $e';
      CustomToastNoti.show(title: 'Error', description: error.value!);
    } finally {
      loading.value = false;
    }
  }
